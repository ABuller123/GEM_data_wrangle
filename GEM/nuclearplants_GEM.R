...
GEM <-read_excel("/Users/2diigermany/Desktop/Work/GEM /Power/Global-Nuclear-Power-Tracker-July-2024.xlsx", sheet = "Data")
  ...
  df %>%
    ...
    mutate(
      value = case_when(
        ...
        !is.na(`Retirement Year`) & year >= `Retirement Year` & `Retirement Year` <= 2050 ~ 0,  # After planned retirement, if within range
...
GEM_timeseries_cleaned <- GEM_timeseries %>%
  select(-c(
    `Planned Retirement`, `Retirement Year`, `Start Year`, `Status`, `GEM unit ID`,
  ))
...
#Capacity_allocated does not necessarily need to be summed here, we are focusing on value
...
#write results
write.csv(GEM_timeseries_aggregated,"/Users/2diigermany/Desktop/Work/GEM /Wrangled/GEM_Nuclear.csv", row.names = F)


#' to note
#' id system is a bit whack but works, might want to kick out and do at the final wrangling
#' Nuclear start year for announced/construction set to 2030 if missing
