...
GEM <-read_excel("/Users/2diigermany/Desktop/Work/GEM /Power/Global-Solar-Power-Tracker-June-2024-v2.xlsx", sheet = "20 MW+", col_types = "text")

GEM_belowthreshold <-read_excel("/Users/2diigermany/Desktop/Work/GEM /Power/Global-Solar-Power-Tracker-June-2024-v2.xlsx", sheet = "1-20 MW", col_types = "text")
...
GEM$`Capacity (MW)` <- as.numeric(GEM$`Capacity (MW)`)
GEM$`Start year` <- as.numeric(GEM$`Start year`)
GEM$`Retired year` <- as.numeric(GEM$`Retired year`)
...
# nearly 30k removed, I assume mostly small plants
GEM_relevant <- GEM_relevant %>%
  filter(!is.na(Owner))
...
GEM_timeseries_cleaned <- GEM_timeseries %>%
  select(-c(
    `Retired year`, `Start year`, `Status`, `GEM phase ID`
  ))

# Aggregate values based on GEM location ID, Owner, and Year
GEM_timeseries_aggregated <- GEM_timeseries_cleaned %>%
  group_by(`GEM location ID`, Owner, `Country`, `Project Name`,Latitude, Longitude, Region, year) %>%
  summarise(
...
#write results
write.csv(GEM_timeseries_aggregated,"/Users/2diigermany/Desktop/Work/GEM /Wrangled/GEM_Solar.csv", row.names = F)

#' to note
#' Solar start year for announced/construction set to 2030 if missing
...
#' Multiple (like 30k) rows removes,  because we do not have owner information
