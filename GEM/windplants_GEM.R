...
GEM <-read_excel("/Users/2diigermany/Desktop/Work/GEM /Power/Global-Wind-Power-Tracker-June-2024.xlsx", sheet = "Data")

GEM_belowthreshold <-read_excel("/Users/2diigermany/Desktop/Work/GEM /Power/Global-Wind-Power-Tracker-June-2024.xlsx", sheet = "Below Threshold")
...
GEM_timeseries_cleaned <- GEM_timeseries %>%
  select(-c(
    `Retired year`, `Start year`, `Status`, `GEM phase ID`,`Installation Type`
  ))
...
#write results
write.csv(GEM_timeseries_aggregated,"/Users/2diigermany/Desktop/Work/GEM /Wrangled/GEM_Wind.csv", row.names = F)

#' to note
#' id system is a bit whack but works, might want to kick out and do at the final wrangling
#' Wind start year for announced/construction set to 2030 if missing
#' Ownership shares are assumed to be equally weighted if missing, (did not miss for all)
#' I might want to check again if Long Lat makes a problem if they are not 100% the same, but its still the same location ID...
